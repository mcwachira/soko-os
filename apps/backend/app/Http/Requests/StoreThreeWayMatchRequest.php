...
class StoreThreeWayMatchRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'purchase_order_id' => ['required', 'uuid', Rule::exists('purchase_orders', 'id')->where('organization_id', $this->user()->organization_id)],
            'grn_id' => ['nullable', 'uuid', Rule::exists('goods_received_notes', 'id')->where('organization_id', $this->user()->organization_id)],
            'supplier_invoice_id' => ['required', 'uuid', Rule::exists('supplier_invoices', 'id')->where('organization_id', $this->user()->organization_id)],
